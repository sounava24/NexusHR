...
export default async function AttendancePage() {
  ...
  // ✅ Prevent build crash (VERY IMPORTANT)
  if (!session?.user?.id) {
    return (
  ...
  // ✅ Handle missing employee safely
  if (!employee) {
    return (
      <div className="text-white p-6 text-center">
        Employee not found.
  ...
  const todayRecord = myLogs.find(
    (r: any) => new Date(r.date).getTime() >= today.getTime()
  )
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      <div>
        ...
        <p className="text-zinc-400 mt-1">
          Track your daily work hours and view attendance history.
        </p>
      </div>
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-zinc-400 text-sm max-w-[200px]">
                Record your start time for the day
              </p>
            </div>
          </button>
        </form>
        ...
        <form action={clockOut} className="flex flex-col">
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-zinc-400 text-sm max-w-[200px]">
                Record your end time for the day
              </p>
            </div>
          </button>
        </form>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-white/5">
              {history.map((record: any) => (
                <tr key={record.id} className="hover:bg-zinc-900/30">
                  {(session.user.role === "ADMIN" ||
                    session.user.role === "HR") && (
